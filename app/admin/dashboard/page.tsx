...
type DashboardData = {
  stats: { portfolio: number; portfolioPublished: number; portfolioDrafts: number; blog: number; published: number; drafts: number; contacts: number; newContacts: number };
  recentPortfolio: Array<{ id: string; title: string; createdAt: string }>;
  recentBlog: Array<{ id: string; title: string; createdAt: string }>;
  recentContacts: Array<{ id: string; fullName: string; service: string; createdAt: string }>;
};
  ...
  return (
    <AdminShell>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        {[
          ["Total Contacts", data?.stats.contacts ?? 0],
          ["New Contacts", data?.stats.newContacts ?? 0],
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-3">
        <AdminCard><h2 className="font-heading text-xl font-bold text-white">Recent Contacts</h2><div className="mt-4 grid gap-3">{data?.recentContacts.map((item) => <p key={item.id} className="text-sm text-[#a0a0b8]">{item.fullName} <span className="text-[#737f96]">- {item.service}</span></p>)}</div></AdminCard>
      ...
      <div className="mt-8 flex flex-wrap gap-4">
        <Link className="rounded-lg bg-[#2563eb] px-5 py-3 font-bold text-white" href="/admin/contacts">View Contacts</Link>
